...
class RealEstate(models.Model):
    ...
    street_block = fields.Char(string='Block Number', length=3, help="Numbre of the building block in the street")
    portal = fields.Char(string='Portal', length=3, help="Portal number.")
    stairs = fields.Char(string='Stairs', length=3, help="Stairs number.")
    floor = fields.Char(string='Floor Number', length=3)
    door = fields.Char(string='Door', length=3, help="Door number.")
    address_complement = fields.Char(string="Address Complement", length=40, help="Any data necessary to complete the address")
    city = fields.Char(string="City", length=30, help="The city, if it is different from the municipality.")
    municipality = fields.Char(string="Municipality", length=30, required=True, help="Name of the Municipality")
    municipality_code = fields.Char(string="Municipality Code", length=5, required=True, help="Municipality code, as given by INE")
    province_code = fields.Char(string='Province Code', length=2, required=True)
    postal_code = fields.Char(string='Postal Code', length=5, required=True)
